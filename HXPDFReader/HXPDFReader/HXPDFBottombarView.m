...
@implementation HXPDFBottombarView
{
    ...
    HXPDFReaderTrackControl *trackControl;
    
    NSMutableDictionary *miniThumbViews;
    
    HXPDFReaderPagebarThumb *pageThumbView;
...
#pragma mark - HXPDFBottombarView class methods
...
#pragma mark - HXPDFBottombarView instance methods
...
- (void)updatePageThumbView:(NSInteger)page
{
    ...
    if (page != pageThumbView.tag) // Only if page number changed
    {
        ...
        HXPDFReaderThumbRequest *request = [HXPDFReaderThumbRequest newForView:pageThumbView document:document page:page size:size];
        
        UIImage *image = [[HXPDFReaderThumbCache sharedInstance] thumbRequest:request priority:YES]; // Request the thumb
...
- (instancetype)initWithFrame:(CGRect)frame document:(HXPDFDocument *)object
{
    ...
    if ((self = [super initWithFrame:frame]))
    {
        ...
        trackControl = [[HXPDFReaderTrackControl alloc] initWithFrame:self.bounds]; // Track control view
...
- (void)layoutSubviews
{
    ...
    if (pageThumbView == nil) // Create the page thumb view when needed
    {
        ...
        pageThumbView = [[HXPDFReaderPagebarThumb alloc] initWithFrame:thumbRect]; // Create the thumb view
    ...
    for (NSInteger thumb = 0; thumb < thumbs; thumb++) // Iterate through needed thumbs
    {
        ...
        HXPDFReaderPagebarThumb *smallThumbView = [miniThumbViews objectForKey:key]; // Thumb view
        ...
        if (smallThumbView == nil) // We need to create a new small thumb view for the page number
        {
            ...
            smallThumbView = [[HXPDFReaderPagebarThumb alloc] initWithFrame:thumbRect small:YES]; // Create a small thumb view
            
            HXPDFReaderThumbRequest *thumbRequest = [HXPDFReaderThumbRequest newForView:smallThumbView document:document page:page size:size];
            
            UIImage *image = [[HXPDFReaderThumbCache sharedInstance] thumbRequest:thumbRequest priority:NO]; // Request the thumb
    ...
    [thumbsToHide enumerateKeysAndObjectsUsingBlock: // Hide unused thumbs
     ...
     {
         HXPDFReaderPagebarThumb *thumb = object; thumb.hidden = YES;
     }
     ];
}
...
- (NSInteger)trackViewPageNumber:(HXPDFReaderTrackControl *)trackView
{
...
- (void)trackViewTouchDown:(HXPDFReaderTrackControl *)trackView
{
...
- (void)trackViewValueChanged:(HXPDFReaderTrackControl *)trackView
{
    NSInteger page = [self trackViewPageNumber:trackView]; // Page
...
- (void)trackViewTouchUp:(HXPDFReaderTrackControl *)trackView
{
    [trackTimer invalidate];
...
//
//    HXPDFReaderTrackControl class implementation
//

@implementation HXPDFReaderTrackControl
{
    CGFloat _value;
}
...
//
//    HXPDFReaderPagebarThumb class implementation
//

@implementation HXPDFReaderPagebarThumb
