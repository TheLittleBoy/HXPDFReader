...
#import "HXPDFDetailContentPage.h"
#import "HXPDFDetailContentTile.h"

@implementation HXPDFDetailContentPage
{
...
+ (Class)layerClass
{
    return [HXPDFDetailContentTile class];
}
...
- (void)highlightPageLinks
{
    if (_links.count > 0) // Add highlight views over all links
    {
        ...
        for (HXPDFDocumentLink *link in _links) // Enumerate the links array
        {
...
- (HXPDFDocumentLink *)linkFromAnnotation:(CGPDFDictionaryRef)annotationDictionary
{
    HXPDFDocumentLink *documentLink = nil; // Document link object
    ...
    if (CGPDFDictionaryGetArray(annotationDictionary, "Rect", &annotationRectArray))
    {
        ...
        documentLink = [HXPDFDocumentLink newWithRect:viewRect dictionary:annotationDictionary];
    }
        ...
        for (NSInteger index = 0; index < count; index++) // Iterate through all annotations
        {
            ...
            if (CGPDFArrayGetDictionary(pageAnnotations, index, &annotationDictionary) == true)
            {
                ...
                if (CGPDFDictionaryGetName(annotationDictionary, "Subtype", &annotationSubtype) == true)
                {
                    ...
                    {
                        HXPDFDocumentLink *documentLink = [self linkFromAnnotation:annotationDictionary];
...
- (id)processSingleTap:(UITapGestureRecognizer *)recognizer
{
    ...
    if (recognizer.state == UIGestureRecognizerStateRecognized)
    {
        if (_links.count > 0) // Process the single tap
        {
            ...
            for (HXPDFDocumentLink *link in _links) // Enumerate links
            {
...
- (instancetype)initWithDocument:(HXPDFDocument *)document page:(NSInteger)page
{
    ...
    if (document != nil) // Check for non-nil file URL
    {
        _PDFDocRef = [document thePDFDocRef];
    ...
    HXPDFDetailContentPage *view = [self initWithFrame:viewRect];
...
{
    HXPDFDetailContentPage *readerContentPage = self; // Retain self
    ...
    CGContextConcatCTM(context, CGPDFPageGetDrawingTransform(_PDFPageRef, kCGPDFCropBox, layer.bounds, 0, true));
...
//
//    HXPDFDocumentLink class implementation
//

@implementation HXPDFDocumentLink
{
...
{
    return [[HXPDFDocumentLink alloc] initWithRect:linkRect dictionary:linkDictionary];
}
